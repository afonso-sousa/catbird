...
class Decoder(nn.Module):
    ...
    def forward(self, input, last_hidden, encoder_outputs):  # Previous Output, the previous step hidden_state
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, trg, teacher_forcing_ratio=0.5):
        ...
        for t in range(1, max_len):
            ...
            top1 = output.data.max(1)[1]  # Follow DIM = 1 Solving the maximum value and maximum index, X [1] is the index of the maximum value=>top1.shape=32
...
class BeamSearchNode(object):
    ...
    def eval(self, alpha=1.0):
        ...
        return self.logp / float(self.leng - 1 + 1e-6) + alpha * reward  # Note that this is a penalty parameter, refer to Na's beam-search

    def __lt__(self, other):
        return self.leng < other.leng  # How to deal with conflicts when the score is displayed, what indicators are specifically used, discussed according to specific circumstances
