...
def draw_edge(
    ...
    final_nodes[accumulate_node + node_raw_id]["index"] = [
        ii + accumulate_word for i in NDE["index"] for ii in reindex_list[i]
    ]

    ## copy existed edges
    for i, edge in enumerate(raw_edges[node_raw_id]):
        if i == node_raw_id:
            final_edges[accumulate_node + node_raw_id][
                accumulate_node + node_raw_id
            ] = "SELF"
        elif edge:
    ...
    for i, node in enumerate(final_nodes[accumulate_node + raw_node_num :]):
        ...
        word_i = [
            ...
            for w in words
            if len(w) > 0
            and w not in stopwords_eng
            and not pattern.fullmatch(w)
        ...
        common1 = [
            w
            for w in word_i
            if w in word_j or w.lower() in [ww.lower() for ww in word_j]
        ]
        common2 = [
            w
            for w in word_j
            if w in word_i or w.lower() in [ww.lower() for ww in word_i]
        ]
        ...
        pos_qualify = (
            NDE["pos"] in verb_pos + noun_pos and node["pos"] in verb_pos + noun_pos
        )
        dep_qualify = (
            NDE["dep"] in subj_and_obj + modifiers
            and node["dep"] in subj_and_obj + modifiers
        )
        ...
        if pos_qualify or dep_qualify:
            if (flag_up or flag_pos) and len(word_i) * len(word_j) > 0:
                ...
                if (
                    max(prb1, prb2) > 1 / 2 and min(prb1, prb2) > 1 / 3
                ):  # requirement of the lavel of overlapping
                    final_edges[accumulate_node + node_raw_id][i_idx] = final_edges[
                        i_idx
                    ][accumulate_node + node_raw_id] = "SIMILAR"
...
def merge_corpus(corpus):
    ...
    sequences, subgraphs = [sent["sequence"] for sent in corpus], [
        sent["graph"] for sent in corpus
    ]
    ...
    for sequence, subgraph in zip(sequences, subgraphs):
        ...
        for idx, node in enumerate(nodes):
            draw_edge(
                ...
                raw_edges=edges,
                accumulate_node=node_cnt,
                accumulate_word=word_cnt,
                reindex_list=indexes,
            )
