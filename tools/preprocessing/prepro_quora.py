...
To get the question features. You will also see some question statistics in the terminal output. This will generate two
files in the current directory, only_quora_tt.h5 and only_quora_tt.json.
Also in this code a lot of places you will find things related to captions, but they actually correspond to paraphrases.
Reuse of previous code :p
"""
import argparse
import json
import re
...
def prepro_question1(imgs, params):
    ...
    for i, img in enumerate(imgs):
        ...
        img[
            "processed_tokens_caption"
        ] = txt_c  # this name is a bit misleading, it is for paraphrase questions actually.
        if i < 10:
            print(txt_c)
        if i % 1000 == 0:
            sys.stdout.write(
...
def build_vocab_question(
    ...
    vocab = [
        w for w, n in counts.items() if n > count_thr
    ]  # will incorpate vocab for  both caption and question
...
def apply_vocab_question(imgs, wtoi):  # this is for val or test question and caption
    ...
    for img in imgs:
        txt = img["processed_tokens"]
        question = [
            w if wtoi.get(w, len(wtoi) + 1) != (len(wtoi) + 1) else "UNK" for w in txt
        ]
        img["final_question"] = question
        txt_c = img["processed_tokens_caption"]
...
def encode_question2(imgs, params, wtoi):
    ...
    caption_arrays = np.zeros(
        (N, max_length), dtype="uint32"
    )  # will store encoding caption words
    caption_length = np.zeros(N, dtype="uint32")  # will store encoding caption words

    for i, img in enumerate(imgs):
        question_id[question_counter] = img["id"]  # unique_id
        label_length[question_counter] = min(
            max_length, len(img["final_question"])
        )  # record the length of this question sequence
        caption_length[question_counter] = min(
            max_length, len(img["final_caption"])
        )  # record the length of this caption sequence
        question_counter += 1
...
def main(params):
    ...
    (
        ...
        cap_train5,
        cap_length_train5,
    ) = encode_question2(imgs_train5, params, wtoi)
    ...
    (
        ...
        cap_test5,
        cap_length_test5,
    ) = encode_question2(imgs_test5, params, wtoi)
    ...
    f.create_dataset(
        "ques_cap_id_train", dtype="uint32", data=question_id_train5
    )  # this is actually the ques_cap_id
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_train_json5",
        default="../data/quora_raw_train.json",
        help="input json file to process into hdf5",
    )
    parser.add_argument(
        "--input_test_json5",
        default="../data/quora_raw_test.json",
        help="input json file to process into hdf5",
    )
    parser.add_argument(
        "--num_ans",
    ...
    parser.add_argument(
        "--output_json5",
        default="../data/quora_data_prepro.json",
        help="output json file",
    )
    parser.add_argument(
        "--output_h55", default="../data/quora_data_prepro.h5", help="output h5 file"
    )
